...
-- Состояния
local track_volumes = {}     -- для текущего файла: id -> vol
local active_tracks = {}     -- id -> true
local current_edit_track = nil
local osd_timer = nil

-- глобальные сохранённые громкости: sig -> vol
...
-- ========== ГОРЯЧИЕ КЛАВИШИ ==========
mp.add_key_binding("KP8", "select-prev-track", function() select_next_track(-1) end)
mp.add_key_binding("KP2", "select-next-track", function() select_next_track(1) end)

-- регулировка громкости
mp.add_key_binding("KP4", "decrease-track-volume", function() adjust_volume(-STEP) end)
mp.add_key_binding("KP6", "increase-track-volume", function() adjust_volume(STEP) end)

-- mute
mp.add_key_binding("KP5", "toggle-mute-track", toggle_mute)

-- быстрые шаги тоже можно на +/- (на numpad они обычно KP_SUBTRACT/KP_ADD)
mp.add_key_binding("KP_SUBTRACT", "large-decrease", function() adjust_volume(-LARGE_STEP) end)
mp.add_key_binding("KP_ADD", "large-increase", function() adjust_volume(LARGE_STEP) end)
